...
task :heroku_maintenance => :environment do
  log_exception() do
    ...
    # heroku scale workers
    log_exception() do
      log_console('STARTING heroku_maintenance - scale workers')
  
      $config.heroku_workers.each do |queue|
        if Delayed::Job.where(:queue => queue, :failed_at => nil).count > 0
          HerokuTools::HerokuTools.scale_workers(queue, 1)
        else
          HerokuTools::HerokuTools.scale_workers(queue, 0)
        end
      end
  
      log_console('FINISHED heroku_maintenance - scale workers')
    end
